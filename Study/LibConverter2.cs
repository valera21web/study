...
namespace LibConvert
{
    public class LibConverter2
    {
        ...
        public LibConverter2(Bitmap image)
        {
            this.imageOriginal = image;
        }

        public LibConverter2(Bitmap image, bool isR, bool isG, bool isB, int countLastBitsToWrite)
        {
            this.imageOriginal = image;
        ...
        #region Set/Get data
        public LibConverter2 SetR(bool isR)
        {
            this.R_Available = (byte)(isR ? _R : _NOT);
            return this;
        }
        public LibConverter2 SetEncodedImage(Bitmap image)
        {
            this.imageModified = image;
            return this;
        }
        public LibConverter2 SetG(bool isG)
        {
            this.G_Available = (byte)(isG ? _G : _NOT);
            return this;
        }
        public LibConverter2 SetB(bool isB)
        {
            this.B_Available = (byte)(isB ? _B : _NOT);
            return this;
        }
        public LibConverter2 SetDataToEncrypt(byte[] data)
        {
            this.dataToEncrypt = data;
            return this;
        }
        public LibConverter2 SetDataToEncrypt(string data)
        {
        ...
        public LibConverter2 AESEncryptData(byte[] key, byte[] iv)
        {
        ...
        public Bitmap Do()
        {
            ...
            foreach (var row in toWrite)
            {
                ...
                var tmp = Color.FromArgb(pixel.A, (pixel.R & 0b11110000) | row[0], (pixel.G & 0b11110000) | row[1], (pixel.B & 0b11110000) | row[2]);
        ...
        public string Decrypt(byte[] key, byte[] iv)
        {
            ...
            for (int i = 0; i < countDataList.Length; )
            {
                ...
                if (i < countDataList.Length)
                    countDataList[i++] = (byte)(0b00001111 & pixelEnc.R);
                if (i < countDataList.Length)
                    countDataList[i++] = (byte)(0b00001111 & pixelEnc.G);
                if (i < countDataList.Length)
                    countDataList[i++] = (byte)(0b00001111 & pixelEnc.B);
                ++x;
            }
